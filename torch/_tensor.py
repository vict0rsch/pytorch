...
class Tensor(torch._C.TensorBase):
    ...
    def dim_order(
        ...
        Args:
            ...
            ... )  # The first dim is singleton, so the dim order is ambiguous.
            RuntimeError: The dim order of the tensor cannot be uniquely determined.
            >>> torch.empty((1, 2, 3, 4)).dim_order(
            ...     ambiguity_check=[torch.contiguous_format, torch.channels_last]
            ... )  # It can be mapped to contiguous format
            (0, 1, 2, 3)
            >>> torch.empty((1, 2, 3, 4)).dim_order(
            ...     ambiguity_check="ILLEGAL_STR"
            ... )  # `ambiguity_check` must be bool or list of memory formats
            TypeError: The ambiguity_check argument must be a bool or a list of memory formats.
        .. warning::
        ...
        if (
            ...
            raise RuntimeError(
                "The dim order of the tensor cannot be uniquely determined or fall back to exact one of the given memory formats."
            )

...
inline std::string getCommsNodeAttrs(const RecordFunction& fn) { // NOLINT
  ...
  // since we currently have this read called in onFunctionExit flow, we should only
  // introspect output tensors to prevent an INTERNAL ASSERT FAILED 
  // in RecordFunction when we try to read input in RecordFunction exit methods.
  auto meta = saveNcclMeta(fn, SaveNcclMetaConfig(true, true, false, true));
